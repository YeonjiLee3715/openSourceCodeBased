...
struct _fluid_midi_event_t {
  ...
  unsigned int param3;      /* Third parameter */
...
typedef struct _fluid_tempo_t fluid_tempo_t;

struct _fluid_evt_with_ticks_t {
    unsigned int ticks;
    fluid_midi_event_t* evt;
};

typedef struct _fluid_evt_with_ticks_t fluid_evt_with_ticks_t;
...
struct _fluid_player_t {
  ...
  fluid_list_t* tempo;				/* tempo list */
  fluid_list_t* cur_tempo;
  int tempo_offset_bpm;				/* for tempo tuning */
  float tempo_offset_timeCoeff;		/* for tempo tuning */
  int total_ticks;
  int total_msec;
  ...
  int play_duration_msec; // real time duration

  fluid_real_t midi_duration_msec;
  int duration_ticks;
  fluid_real_t tick_duration_msec;
  int last_sync_msec;
  ...
  double playdeltatime;     /* milliseconds per midi tick. depends on set-tempo */
  unsigned int division;

  handle_midi_event_func_t playback_callback; /* function fired on each midi event as it is played */
  handle_midi_seek_event_func_t seeking_callback; /* function fired on each midi event as it is played */
  void* playback_userdata; /* pointer to user-defined data passed to playback_callback function */
  int play_next_auto;

  int is_seeking;
  fluid_mutex_t player_mutex;
};

int fluid_player_add_track(fluid_player_t *player, fluid_track_t *track);
int fluid_player_add_temp_list(fluid_player_t *player, fluid_list_t *tempo_list);
...
int fluid_midi_file_read_event(fluid_midi_file* mf, fluid_track_t* track, fluid_list_t** tempo_list, int* trackTicks, fluid_list_t** on_note_list );
