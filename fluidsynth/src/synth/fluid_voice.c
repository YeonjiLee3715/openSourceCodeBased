...
new_fluid_voice(fluid_real_t output_rate)
{
  ...
  if (voice->rvoice == NULL || voice->overflow_rvoice == NULL) {
    ...
    if( voice->overflow_rvoice != NULL ) {
      FLUID_FREE(voice->overflow_rvoice);
      voice->overflow_rvoice = NULL;
    }
  ...
  voice->channel = NULL;
  voice->sample = NULL;
  voice->preset_amplitude_offset = 1.0;
  voice->mute = FALSE;
...
fluid_voice_init(fluid_voice_t* voice, fluid_sample_t* sample,
		 ...
		 unsigned int start_time, fluid_real_t gain, fluid_real_t amp_offset)
{
  ...
  voice->preset_amplitude_offset = amp_offset;
  /* avoid division by zero later*/
  if (voice->preset_amplitude_offset < 0.0000001){
    voice->preset_amplitude_offset = 0.0000001;
  }

  voice->chan_gain = channel->chan_gain;
  if (voice->chan_gain < 0.0000001){
    voice->chan_gain = 0.0000001;
  }

  UPDATE_RVOICE_R1(fluid_rvoice_set_synth_gain, voice->synth_gain);
  UPDATE_RVOICE_R1(fluid_rvoice_set_preset_amplitude_offset, voice->preset_amplitude_offset);
  UPDATE_RVOICE_R1(fluid_rvoice_set_channel_gain, voice->chan_gain);
		 ...
		 fluid_real_t* reverb_buf, fluid_real_t* chorus_buf)
{
  ...
  if ( voice->mute == TRUE ){
    fluid_rvoice_buffers_set_amp(&buffers, 0, 0.0f);
    fluid_rvoice_buffers_set_amp(&buffers, 1, 0.0f);
    fluid_rvoice_buffers_set_amp(&buffers, 2, 0.0f);
    fluid_rvoice_buffers_set_amp(&buffers, 3, 0.0f);
  }
  else
  {
...
fluid_voice_update_param(fluid_voice_t* voice, int gen)
{
  ...
  case GEN_PAN:
    ...
    voice->amp_left = fluid_pan(voice->pan, 1) * (voice->synth_gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
    voice->amp_right = fluid_pan(voice->pan, 0) * (voice->synth_gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
    if ( voice->mute == TRUE ){
      UPDATE_RVOICE_BUFFERS2(fluid_rvoice_buffers_set_amp, 0, 0.0f);
      UPDATE_RVOICE_BUFFERS2(fluid_rvoice_buffers_set_amp, 1, 0.0f);
    }
    else {
  ...
  case GEN_REVERBSEND:
    ...
    voice->amp_reverb = voice->reverb_send * (voice->synth_gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
    if ( voice->mute == TRUE ){
      UPDATE_RVOICE_BUFFERS2(fluid_rvoice_buffers_set_amp, 2, 0.0f);
    }
    else {
  ...
  case GEN_CHORUSSEND:
    ...
    voice->amp_chorus = voice->chorus_send * (voice->synth_gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
    if ( voice->mute == TRUE ){
      UPDATE_RVOICE_BUFFERS2(fluid_rvoice_buffers_set_amp, 3, 0.0f);
    }
    else {
...
void
fluid_voice_update_mute_status(fluid_voice_t* voice)
{
  fluid_channel_t* channel;
...
int fluid_voice_set_gain(fluid_voice_t* voice, fluid_real_t gain)
{
  ...
  voice->synth_gain = gain;
  voice->amp_left = fluid_pan(voice->pan, 1) * (gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_right = fluid_pan(voice->pan, 0) * (gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_reverb = voice->reverb_send * (gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_chorus = voice->chorus_send * (gain*voice->preset_amplitude_offset*voice->chan_gain) / 32768.0f;
...
int fluid_voice_set_preset_amplitude_offset(fluid_voice_t* voice, fluid_real_t preset_amplitude_offset)
{
  /* avoid division by zero*/
  if (preset_amplitude_offset < 0.0000001){
    preset_amplitude_offset = 0.0000001;
  }

  voice->preset_amplitude_offset = preset_amplitude_offset;
  voice->amp_left = fluid_pan(voice->pan, 1) * (voice->synth_gain*preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_right = fluid_pan(voice->pan, 0) * (voice->synth_gain*preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_reverb = voice->reverb_send * (voice->synth_gain*preset_amplitude_offset*voice->chan_gain) / 32768.0f;
  voice->amp_chorus = voice->chorus_send * (voice->synth_gain*preset_amplitude_offset*voice->chan_gain) / 32768.0f;

  UPDATE_RVOICE_R1(fluid_rvoice_set_preset_amplitude_offset, preset_amplitude_offset);
...
int fluid_voice_set_channel_gain(fluid_voice_t* voice, fluid_real_t chan_gain)
{
  /* avoid division by zero*/
  if (chan_gain < 0.0000001){
    chan_gain = 0.0000001;
  }

  voice->chan_gain = chan_gain;
  voice->amp_left = fluid_pan(voice->pan, 1) * (voice->synth_gain*voice->preset_amplitude_offset*chan_gain) / 32768.0f;
  voice->amp_right = fluid_pan(voice->pan, 0) * (voice->synth_gain*voice->preset_amplitude_offset*chan_gain) / 32768.0f;
  voice->amp_reverb = voice->reverb_send * (voice->synth_gain*voice->preset_amplitude_offset*chan_gain) / 32768.0f;
  voice->amp_chorus = voice->chorus_send * (voice->synth_gain*voice->preset_amplitude_offset*chan_gain) / 32768.0f;

  UPDATE_RVOICE_R1(fluid_rvoice_set_channel_gain, chan_gain);
