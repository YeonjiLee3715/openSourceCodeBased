...
static int fluid_synth_noteon_LOCAL(fluid_synth_t* synth, int chan, int key,
                                       int vel);
static int fluid_synth_noteon_with_seek_LOCAL(fluid_synth_t* synth, int chan, int key,
                                    int vel, int ticks);
...
static int fluid_synth_sysex_midi_roland_tuning (fluid_synth_t *synth, const char *data,
                                          int len, char *response,
                                          int *response_len, int avail_response,
                                          int *handled, int dryrun);
...
static int fluid_synth_set_channel_mute_LOCAL(fluid_synth_t* synth, int chan, int mute);
...
static void fluid_synth_update_preset_amp_offset_all(fluid_synth_t* synth);
static void fluid_synth_update_preset_amp_offset(fluid_synth_t* synth, int bank_num, int prog_num);
static void fluid_synth_update_channel_preset_amp_offset(fluid_synth_t* synth, int chan, int bank_num, int prog_num);
...
void fluid_synth_settings(fluid_settings_t* settings)
{
  ...
  fluid_settings_register_int(settings, "synth.reverb.active", 1, 0, 1,
                              FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.reverb.room-size", FLUID_REVERB_DEFAULT_ROOMSIZE, 0.0f, 1.0f, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.reverb.damp", FLUID_REVERB_DEFAULT_DAMP, 0.0f, 1.0f, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.reverb.width", FLUID_REVERB_DEFAULT_WIDTH, 0.0f, 100.0f, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.reverb.level", FLUID_REVERB_DEFAULT_LEVEL, 0.0f, 1.0f, FLUID_HINT_TOGGLED, NULL, NULL);

  fluid_settings_register_int(settings, "synth.chorus.active", 1, 0, 1,
                                FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_int(settings, "synth.chorus.nr", FLUID_CHORUS_DEFAULT_N, 0, 99, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.chorus.level", FLUID_CHORUS_DEFAULT_LEVEL, 0.0f, 10.0f, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.chorus.speed", FLUID_CHORUS_DEFAULT_SPEED, 0.1f, 5.0f, FLUID_HINT_TOGGLED, NULL, NULL);
  fluid_settings_register_num(settings, "synth.chorus.depth", FLUID_CHORUS_DEFAULT_DEPTH, 0.0f, 256.0f, FLUID_HINT_TOGGLED, NULL, NULL);
...
fluid_sample_timer_t* new_fluid_sample_timer(fluid_synth_t* synth, fluid_timer_callback_t callback, void* data)
{
    ...
    FLUID_MEMSET (result, 0, sizeof (fluid_sample_timer_t));
...
int fluid_sample_timer_seek(fluid_synth_t* synth, fluid_sample_timer_t* timer, int msec)
{
    if( synth == NULL || timer == NULL )
        return FLUID_FAILED;

    timer->starttick = fluid_synth_get_ticks(synth)-(int)(((double)msec)/1000.0*synth->sample_rate);
...
new_fluid_synth(fluid_settings_t *settings)
{
  ...
  synth->preset_amp_offset_hash = new_fluid_hashtable_full( fluid_str_hash, fluid_str_equal, delete_fluid_string_key, delete_fluid_preset_amp_offset_list_all );
  ...
  /* set channel gain */
  synth->channel_gain = FLUID_ARRAY(float, synth->midi_channels);
  ...
  for (i = 0; i < synth->midi_channels; i++) {
    ...
    synth->channel_gain[i] = 1.0f;
  }

  /* set mute */
  synth->mute = FLUID_ARRAY(int, synth->midi_channels);
  FLUID_MEMSET( synth->mute, FALSE, sizeof(int)*synth->midi_channels );

  /* set solo */
  synth->solo = FLUID_ARRAY(int, synth->midi_channels);
  FLUID_MEMSET( synth->solo, FALSE, sizeof(int)*synth->midi_channels );
  ...
  synth->dither_index = 0;

    {
        ...
        fluid_settings_getnum(synth->settings, "synth.reverb.room-size", &room);
        fluid_settings_getnum(synth->settings, "synth.reverb.damp", &damp);
        fluid_settings_getnum(synth->settings, "synth.reverb.width", &width);
        fluid_settings_getnum(synth->settings, "synth.reverb.level", &level);
...

    {
        ...
        fluid_settings_getint(synth->settings, "synth.chorus.nr", &i);
        fluid_settings_getnum(synth->settings, "synth.chorus.level", &level);
        fluid_settings_getnum(synth->settings, "synth.chorus.speed", &speed);
        fluid_settings_getnum(synth->settings, "synth.chorus.depth", &depth);
...
int
fluid_synth_event_reset(fluid_synth_t *synth)
{
    int result;
    int idx = 0;
    ...
    for( idx = 0; idx < synth->midi_channels; ++idx ) {
        synth->channel_gain[idx] = 1.0f;
    }

    fluid_synth_system_reset( synth );
    fluid_synth_program_reset( synth );

    fluid_rvoice_mixer_event_reset( synth->eventhandler->mixer );
    ...
    if(fluid_settings_str_equal(synth->settings, "synth.midi-bank-select", "gm"))
    {
        synth->bank_select = FLUID_BANK_STYLE_GM;
    }
    else if(fluid_settings_str_equal(synth->settings, "synth.midi-bank-select", "gs"))
    {
        synth->bank_select = FLUID_BANK_STYLE_GS;
    }
    else if(fluid_settings_str_equal(synth->settings, "synth.midi-bank-select", "xg"))
    {
        synth->bank_select = FLUID_BANK_STYLE_XG;
    }
    else if(fluid_settings_str_equal(synth->settings, "synth.midi-bank-select", "mma"))
    {
        synth->bank_select = FLUID_BANK_STYLE_MMA;
    }
...
int
fluid_synth_set_mute(fluid_synth_t* synth, int chan, int mute)
{
    ...
    fluid_return_val_if_fail(synth->mute != NULL, FLUID_FAILED);
    ...
    if (chan >= synth->midi_channels) {
        ...
        synth->mute[chan] = mute;

        for( idx = 0; idx < synth->midi_channels; ++idx ) {
            if( synth->solo[idx] == TRUE ) {
                soloExist = TRUE;
                break;
            }
        }
        ...
        if( soloExist == FALSE ) {
            result = fluid_synth_set_channel_mute_LOCAL( synth, chan, mute );
        }
    }
...
int
fluid_synth_set_solo(fluid_synth_t* synth, int chan, int solo)
{
    int result = FLUID_FAILED;
    int idx = 0;
    int soloExist = FALSE;
    int switchStatus = FALSE;
    ...
    if (chan >= synth->midi_channels) {
        ...
        if( synth->solo[chan] != solo ) {
            synth->solo[chan] = solo;
            switchStatus = TRUE;
        }

        for( idx = 0; idx < synth->midi_channels; ++idx ) {
            if( idx != chan ) {
                if( synth->solo[idx] == solo ) {
                    switchStatus = FALSE;
                }
            }
        ...
        if( switchStatus == FALSE ) {
            if( soloExist == FALSE )
                result = fluid_synth_set_channel_mute_LOCAL( synth, chan, synth->mute[chan] );
            else
                result = fluid_synth_set_channel_mute_LOCAL( synth, chan, (synth->solo[chan]==FALSE?TRUE:FALSE) );
        } else {
            for( idx = 0; idx < synth->midi_channels; ++idx ) {
                if( soloExist == FALSE ) {
                    result = fluid_synth_set_channel_mute_LOCAL( synth, chan, synth->mute[idx] );
                }
                else {
                    result = fluid_synth_set_channel_mute_LOCAL( synth, chan, ( synth->solo[idx] == FALSE ? TRUE : FALSE ) );
                }
            }
        }
    }
...
delete_fluid_synth(fluid_synth_t* synth)
{
  ...
  fluid_sfloader_t* loader;
  fluid_channel_t* del_channel = NULL;
  fluid_voice_t* del_voice = NULL;
  fluid_tuning_t* del_tuning = NULL;
  ...
  if (synth->channel != NULL) {
    for (i = 0; i < synth->midi_channels; i++) {
        del_channel = synth->channel[i];
        synth->channel[i] = NULL;
        if (del_channel != NULL) {
	        delete_fluid_channel(del_channel);
            del_channel = NULL;
        }
    }
  ...
  if (synth->voice != NULL) {
    for (i = 0; i < synth->nvoice; i++) {
        del_voice = synth->voice[i];
        synth->voice[i] = NULL;
        if (del_voice != NULL) {
            delete_fluid_voice(del_voice);
            del_voice = NULL;
        }
    }
  ...
  if (synth->tuning != NULL) {
	...
	for (k = 0; k < 128; k++) {
	  if (synth->tuning[i][k] != NULL) {
        del_tuning = synth->tuning[i][k];
        synth->tuning[i][k] = NULL;
	    delete_fluid_tuning(del_tuning);
        del_tuning = NULL;
	  }
	}
       del_tuning = synth->tuning[i];
	   synth->tuning[i] = NULL;
       if( del_tuning != NULL ) {
          FLUID_FREE(del_tuning);
          del_tuning = NULL;
       }
      }
    }
  ...
  if( synth->solo != NULL ) {
      FLUID_FREE(synth->solo);
      synth->solo = NULL;
  }

  if( synth->mute != NULL ) {
      FLUID_FREE(synth->mute);
      synth->mute = NULL;
  }

  if( synth->channel_gain != NULL ) {
      FLUID_FREE(synth->channel_gain);
      synth->channel_gain = NULL;
  }
 ...
 */
int
fluid_synth_noteon_with_seek(fluid_synth_t* synth, int chan, int key, int vel, int ticks)
{
    int result;
    fluid_return_val_if_fail (key >= 0 && key <= 127, FLUID_FAILED);
    fluid_return_val_if_fail (vel >= 0 && vel <= 127, FLUID_FAILED);
    ...
    result = fluid_synth_noteon_with_seek_LOCAL (synth, chan, key, vel, ticks);
    FLUID_API_RETURN(result);
}

/* Local synthesis thread variant of fluid_synth_noteon */
static int
fluid_synth_noteon_with_seek_LOCAL(fluid_synth_t* synth, int chan, int key, int vel, int ticks)
{
    fluid_channel_t* channel;
    ...
    return fluid_preset_noteon_with_seek(channel->preset, synth, chan, key, vel, ticks);
}
                  ...
                  char *response, int *response_len, int *handled, int dryrun)
{
  ...
  else if(data[0] == MIDI_SYSEX_MANUF_ROLAND)
  {
      fluid_synth_api_enter(synth);
      result = fluid_synth_sysex_midi_roland_tuning (synth, data, len, response,
                                              response_len, avail_response,
                                              handled, dryrun);
      FLUID_API_RETURN(result);
  }
...
static uint16_t fluid_gs_convert_master_vol(int vol)
{
    double v;
...
static uint16_t fluid_gm_convert_master_vol(uint16_t v1, uint16_t v2)
{
...
static int
fluid_synth_sysex_midi_roland_tuning (fluid_synth_t *synth, const char *data, int len,
                               ...
                               int *handled, int dryrun)
{
       ...
       data[3] == 0x12)   /* Data Set Command */
    {
        ...
        int32_t addr, checksum;		/* SysEx address */
        uint8_t* body;		            /* SysEx body */

        /* check Checksum */
        checksum = 0;
        for( idx=4; idx < len-1; ++idx ) {
            checksum += data[idx];
        }
        if(((128 - (checksum & 0x7F)) & 0x7F) != data[len-1]) {
            return FLUID_FAILED;
        }

        addr = (((int32_t)data[4]) << 16 |
                ((int32_t)data[5]) << 8 |
                (int32_t)data[6]);
        body = data + 7;
        channels = (int)((addr >> 8) & 0xF);
        if(channels == 0)
            channels = 9;
        else if(channels <= 9)
            channels--;

        if(data[4] == 0x50) {	/* for double module mode */
            channels += 16;
        ...
        {
            if( synth->channel[channels]->channel_type == CHANNEL_TYPE_DRUM )
                fluid_synth_set_channel_type(synth, channels, CHANNEL_TYPE_MELODIC);
            else if( synth->channel[channels]->channel_type == CHANNEL_TYPE_MELODIC )
                fluid_synth_set_channel_type(synth, channels, CHANNEL_TYPE_DRUM);
            if (handled) *handled = TRUE;
            return FLUID_OK;
        }

        if((addr & 0xFFF0FF) == 0x401016) /* Pitch Key Shift */
        {
            for (idx = 0; idx < 12; ++idx) {
                tunedata[idx] = (int)(*body)-(int)0x40;
            }

            if (fluid_synth_activate_octave_tuning (synth, 0, 0, "SYSEX",
                                                    tunedata, TRUE) == FLUID_FAILED)
                return FLUID_FAILED;

            fluid_synth_activate_tuning (synth, channels, 0, 0, TRUE);
...
//            SETMIDIEVENT(ev, 0, ME_MASTER_TUNING, 0, tune & 0xFF, (tune >> 8) & 0x7F);
        ...
        {
            vol = fluid_gs_convert_master_vol(*body);
            fluid_settings_setnum(synth, "synth.gain", (int32_t)vol&0xFF+256*(int32_t)(vol >> 8)&0xFF);
            if (handled) *handled = TRUE;
            return FLUID_OK;
        }
        ...
        {
            if( synth->mute[channels] == TRUE )
                fluid_synth_set_mute( synth, channels, FALSE );
            else if( synth->mute[channels] == FALSE )
                fluid_synth_set_mute( synth, channels, TRUE );
            if (handled) *handled = TRUE;
            return FLUID_OK;
        }
        ...
        {
#if 0
            SETMIDIEVENT(ev, 0, ME_RECEIVE_CHANNEL, (uint8)chan, *body >= 64, SYSEX_TAG);
#endif
            if (handled) *handled = TRUE;
            return FLUID_OK;
        }
...
//
//                tunedata[idx] = note * 100.0 + (frac * 100.0 / 16384.0);
//                ++idx;
//            }
//
//            if (index > 0)
//            {
//                if (fluid_synth_tune_notes (synth, bank, prog, idx, keys, tunedata,
//                                            TRUE) == FLUID_FAILED)
//                    return FLUID_FAILED;
//            }
//            if (handled) *handled = TRUE;
...
//            check_chorus_text_start();
       ...
       data[6] == 0x00)
    {
        ...
        save = data[len];
//        data[len] = '\0';
//        if(readmidi_make_string_event(ME_INSERT_TEXT, (char *)data + 7, ev, 1))
//        {
//            data[len] = save;
//            return FLUID_OK;
//        }
//        data[len] = save;
       ...
       data[6] == 0x00)
    {
        ...
        save = data[len];
//        data[len] = '\0';
//        if(readmidi_make_lcd_event(ME_GSLCD, (uint8 *)data + 7, ev))
//        {
//            data[len] = save;
//            return FLUID_OK;
//        }
...
static int
fluid_synth_set_channel_mute_LOCAL(fluid_synth_t* synth, int chan, int mute)
{
    int result;
    fluid_voice_t* voice;
    int i;
    ...
    if (chan >= synth->midi_channels) {
        ...
        synth->channel[chan]->mute = mute;

        for (i = 0; i < synth->polyphony; i++) {
            voice = synth->voice[i];

            if (fluid_voice_is_playing(voice) && ((-1 == chan) || (chan == fluid_voice_get_channel(voice))))
                fluid_voice_update_mute_status(voice);
        }
    }
    FLUID_API_RETURN(result);
...
fluid_synth_program_change(fluid_synth_t* synth, int chan, int prognum)
{
  ...
  fluid_synth_update_channel_preset_amp_offset( synth, chan, banknum, prognum );
  FLUID_API_RETURN(result);
}
...
fluid_synth_bank_select(fluid_synth_t* synth, int chan, unsigned int bank)
{
  ...
  fluid_synth_update_channel_preset_amp_offset( synth, chan, bank, -1 );
  FLUID_API_RETURN(FLUID_OK);
}
			   ...
			   unsigned int bank_num, unsigned int preset_num)
{
  ...
  fluid_synth_update_channel_preset_amp_offset( synth, chan, bank_num, preset_num );
...
fluid_synth_update_presets(fluid_synth_t* synth)
{
  ...
  for (chan = 0; chan < synth->midi_channels; chan++) {
    ...
    fluid_synth_update_channel_preset_amp_offset( synth, chan, bank, prog );
  }
}
...
fluid_synth_alloc_voice(fluid_synth_t* synth, fluid_sample_t* sample, int chan, int key, int vel)
{
  ...
  unsigned int ticks;
  fluid_real_t value = 1.0;
  fluid_list_t* list = NULL;
  fluid_preset_amp_offset_t* preset_amp_offset = NULL;
  int chan_sfont = 0;
  int chan_bank_num = 0;
  int chan_prog_num = 0;
  int idx = 0;
  ...
  fluid_channel_get_sfont_bank_prog(channel, &chan_sfont, &chan_bank_num, &chan_prog_num);
  value = 1.0;

  for( list = synth->preset_amp_offset_list; list != NULL; list = fluid_list_next (list) )
  {
    if( list->data == NULL )
        continue;

    preset_amp_offset = ( fluid_preset_amp_offset_t* )fluid_list_get ( list );
    if( chan_bank_num == preset_amp_offset->bank_num && chan_prog_num == preset_amp_offset->preset_num )
    {
        value = preset_amp_offset->amp_offset;
        break;
    }
  }

  if (fluid_voice_init (voice, sample, channel, key, vel,
                        synth->storeid, ticks, synth->gain, value) != FLUID_OK) {
 ...
 */
void
fluid_synth_start_voice_with_seek(fluid_synth_t* synth, fluid_voice_t* voice, int ticks)
{
    fluid_return_if_fail (synth != NULL);
    fluid_return_if_fail (voice != NULL);
//  fluid_return_if_fail (fluid_synth_is_synth_thread (synth));
    ...
    if (synth->eventhandler->is_threadsafe)
        fluid_voice_lock_rvoice(voice);
    if( ticks > 0 )
        fluid_rvoice_seek_to_ticks( voice->rvoice, ticks );
    fluid_rvoice_eventhandler_add_rvoice(synth->eventhandler, voice->rvoice);
...
fluid_synth_sfload(fluid_synth_t* synth, const char* filename, int reset_presets)
{
  ...
  for (list = synth->loaders; list; list = fluid_list_next(list)) {
    ...
    if (sfont != NULL) {
      ...
      synth->preset_amp_offset_list = fluid_hashtable_lookup( synth->preset_amp_offset_hash, filename );
...
new_fluid_sfont_info (fluid_synth_t *synth, fluid_sfont_t *sfont)
{
  ...
  FLUID_MEMSET (sfont_info, 0, sizeof (fluid_sfont_info_t));
...
fluid_synth_sfunload(fluid_synth_t* synth, unsigned int id, int reset_presets)
{
  ...
  synth->preset_amp_offset_list = NULL;
...
fluid_preset_amp_offset_t* new_fluid_preset_amp_offset()
{
    fluid_preset_amp_offset_t* result = FLUID_NEW(fluid_preset_amp_offset_t);
    if (result == NULL) {
        FLUID_LOG(FLUID_ERR, "Out of memory");
        return NULL;
    }
    FLUID_MEMSET( result, 0, sizeof(fluid_preset_amp_offset_t) );

    result->amp_offset = 1.0f;

    return result;
}

int fluid_preset_amp_offset_set_infos( fluid_preset_amp_offset_t* preset_amp_offset
        , unsigned int bank_num, unsigned int preset_num, double amp_offset )
{
    fluid_return_val_if_fail (preset_amp_offset != NULL, FLUID_FAILED);

    preset_amp_offset->bank_num = bank_num;
    preset_amp_offset->preset_num = preset_num;
    preset_amp_offset->amp_offset = amp_offset;
}

void delete_fluid_preset_amp_offset( fluid_preset_amp_offset_t* preset_amp_offset )
{
    fluid_return_if_fail(preset_amp_offset != NULL);
    free(preset_amp_offset);
}
...
void delete_fluid_preset_amp_offset_list(void* data)
{
    fluid_list_t* list = (fluid_list_t*)data;
    fluid_preset_amp_offset_t* preset_amp_offset = NULL;
    ...
    if( list->data != NULL )
    {
        preset_amp_offset = (fluid_preset_amp_offset_t*)list->data;
        delete_fluid_preset_amp_offset( preset_amp_offset );
        preset_amp_offset = NULL;
    }
...
void delete_fluid_preset_amp_offset_list_all(void* data)
{
    fluid_list_t* list = (fluid_list_t*)data;
    fluid_list_t* next = NULL;
    fluid_preset_amp_offset_t* preset_amp_offset = NULL;
 ...
 */
int
fluid_synth_handle_midi_seek_event(void* data, fluid_midi_event_t* event, int ticks)
{
    fluid_synth_t *synth = (fluid_synth_t *) data;
    int type = fluid_midi_event_get_type(event);
    int chan = fluid_midi_event_get_channel(event);
    ...
    {
        case NOTE_ON:
            return fluid_synth_noteon_with_seek(synth, chan,
                                      fluid_midi_event_get_key(event),
                                      fluid_midi_event_get_velocity(event), ticks);
...
static void
fluid_synth_update_preset_amp_offset_all(fluid_synth_t* synth)
{
    fluid_voice_t *voice;
    fluid_real_t value = 1.0;
    fluid_list_t* list = NULL;
    ...
    for( idx = 0; idx < synth->polyphony; ++idx )
    {
        voice = synth->voice[idx];

        if( fluid_voice_is_playing(voice) != TRUE )
            continue;

        fluid_channel_get_sfont_bank_prog(voice->channel, &chan_sfont, &chan_bank_num, &chan_prog_num);
        value = 1.0;
        ...
        if( voice->preset_amplitude_offset != value )
            fluid_voice_set_preset_amplitude_offset( voice, value );
    }
}

static void
fluid_synth_update_preset_amp_offset(fluid_synth_t* synth, int bank_num, int prog_num)
{
    fluid_voice_t* voice;
    fluid_real_t value = 1.0;
    fluid_list_t* list = NULL;
    ...
    for( list = synth->sfont_info; list; list = fluid_list_next(list))
    {
        ...
        if (fluid_sfont_get_id (sfont_info->sfont) == synth->sfont_id)
        {
            bank_offset = sfont_info->bankofs;
            break;
        }
    }
    ...
    for( list = synth->preset_amp_offset_list; list != NULL; list = fluid_list_next (list) )
    {
        ...
        preset_amp_offset = ( fluid_preset_amp_offset_t* )fluid_list_get ( list );
        if( preset_amp_offset->bank_num == (bank_num-bank_offset) && prog_num == preset_amp_offset->preset_num )
        {
            value = preset_amp_offset->amp_offset;
            break;
        }
    ...
    for( idx = 0; idx < synth->polyphony; ++idx )
    {
        ...
        if( chan_bank_num == (bank_num-bank_offset) && chan_prog_num == prog_num && fluid_voice_is_playing(voice) )
        {
            if( voice->preset_amplitude_offset != value )
                fluid_voice_set_preset_amplitude_offset( voice, value );
        }
...
static void
fluid_synth_update_channel_preset_amp_offset(fluid_synth_t* synth, int chan, int bank_num, int prog_num)
{
    fluid_voice_t *voice;
    fluid_real_t value = 1.0;
    fluid_list_t* list = NULL;
    ...
    for( list = synth->preset_amp_offset_list; list != NULL; list = fluid_list_next (list) )
    {
        ...
        preset_amp_offset = ( fluid_preset_amp_offset_t* )fluid_list_get ( list );
        if( preset_amp_offset->bank_num == (bank_num-bank_offset) && preset_amp_offset->preset_num == prog_num )
        {
            value = preset_amp_offset->amp_offset;
            break;
        }
...
int fluid_synth_set_preset_amp_offsets(fluid_synth_t* synth, const char *name, fluid_list_t* preset_amp_offset_list)
{
    fluid_sfont_t* sfont = NULL;

    fluid_return_val_if_fail (synth != NULL, 0);
    fluid_return_val_if_fail (name != NULL, 0);
    fluid_return_val_if_fail (preset_amp_offset_list != NULL, 0);
    fluid_synth_api_enter(synth);

    if( fluid_hashtable_lookup( synth->preset_amp_offset_hash, name ) != NULL )
        fluid_hashtable_insert( synth->preset_amp_offset_hash, (void*)name, preset_amp_offset_list );
    else
        fluid_hashtable_insert( synth->preset_amp_offset_hash, FLUID_STRDUP(name), preset_amp_offset_list );

    sfont = fluid_synth_get_sfont_by_id( synth, synth->sfont_id );
    fluid_sfont_get_name( sfont );
    if( FLUID_STRCMP( name, fluid_sfont_get_name( sfont ) ) == 0 )
    {
        synth->preset_amp_offset_list = preset_amp_offset_list;
        fluid_synth_update_preset_amp_offset_all( synth );
    }

    FLUID_API_RETURN(FLUID_OK);
}

int fluid_synth_set_preset_amp_offset(fluid_synth_t* synth, const char *name, int bank_num, int preset_num, double offset)
{
    fluid_sfont_t* sfont = NULL;
    fluid_list_t* preset_amp_offset_list = NULL;
    fluid_preset_amp_offset_t* preset_amp_offset = NULL;
    fluid_list_t* list = NULL;
    int isSet = FALSE;
    fluid_sfont_info_t *sfont_info;
    ...
    preset_amp_offset_list = (fluid_list_t*)fluid_hashtable_lookup( synth->preset_amp_offset_hash, name );

    if( preset_amp_offset_list != NULL )
    {
        for( list = synth->sfont_info; list; list = fluid_list_next(list))
        {
            sfont_info = (fluid_sfont_info_t *)fluid_list_get (list);
        ...
        for (list = preset_amp_offset_list; list != NULL; list = fluid_list_next (list))
        {
            if( list->data == NULL )
                continue;

            preset_amp_offset = (fluid_preset_amp_offset_t *)fluid_list_get (list);
            if( preset_amp_offset->bank_num == (bank_num-bank_offset)
                && preset_amp_offset->preset_num == preset_num )
            {
                preset_amp_offset->amp_offset = offset;
                isSet = TRUE;
                break;
            }
        ...
        if( isSet == FALSE )
        {
            preset_amp_offset = new_fluid_preset_amp_offset();
            fluid_preset_amp_offset_set_infos( preset_amp_offset, bank_num, preset_num, offset );
            fluid_list_append( preset_amp_offset_list, preset_amp_offset );
        }
    }
    else
    {
        preset_amp_offset = new_fluid_preset_amp_offset();
        fluid_preset_amp_offset_set_infos( preset_amp_offset, bank_num, preset_num, offset );
        preset_amp_offset_list = fluid_list_append( preset_amp_offset_list, preset_amp_offset );
    ...
    {
        synth->preset_amp_offset_list = preset_amp_offset_list;
        fluid_synth_update_preset_amp_offset( synth, bank_num, preset_num );
    }

    FLUID_API_RETURN(FLUID_OK);
}

fluid_list_t* fluid_synth_get_preset_amp_offsets(fluid_synth_t* synth, const char* name )
{
    fluid_return_val_if_fail (synth != NULL, NULL);
    fluid_return_val_if_fail (name != NULL, NULL);

    return (fluid_list_t*)fluid_hashtable_lookup( synth->preset_amp_offset_hash, name );
}

double fluid_synth_get_preset_amp_offset(fluid_synth_t* synth, const char* name, int bank_num, int preset_num )
{
    fluid_list_t* preset_amp_offset_list = NULL;
    fluid_preset_amp_offset_t* preset_amp_offset = NULL;
    fluid_list_t* list = NULL;
    ...
    if( preset_amp_offset_list != NULL )
    {
        ...
        for (list = preset_amp_offset_list; list != NULL; list = fluid_list_next (list))
        {
            ...
            preset_amp_offset = (fluid_preset_amp_offset_t *)fluid_list_get (list);
            if( preset_amp_offset->bank_num == (bank_num-bank_offset) && preset_amp_offset->preset_num == preset_num )
                return preset_amp_offset->amp_offset;
        }
    }
...
void fluid_synth_delete_preset_amp_offsets(fluid_synth_t* synth, const char* name)
{
    fluid_list_t* preset_amp_offset_list = NULL;
    int bNeedUpdate = FALSE;
    ...
    if( preset_amp_offset_list == synth->preset_amp_offset_list )
    {
        synth->preset_amp_offset_list = NULL;
        bNeedUpdate = TRUE;
    }

    delete_fluid_preset_amp_offset_list_all( preset_amp_offset_list );
...
void fluid_synth_delete_preset_amp_offset(fluid_synth_t* synth, const char *name, int bank_num, int preset_num)
{
    fluid_list_t* preset_amp_offset_list = NULL;
    fluid_preset_amp_offset_t* preset_amp_offset = NULL;
    fluid_list_t* preList = NULL;
    fluid_list_t* list = NULL;
    int bNeedUpdate = FALSE;
    fluid_sfont_info_t *sfont_info;
    int bank_offset = 0;
        ...
        for (list = preset_amp_offset_list; list != NULL; list = fluid_list_next (list))
        {
            if( list->data != NULL )
            {
                ...
                if( preset_amp_offset->bank_num == (bank_num-bank_offset)
                    && preset_amp_offset->preset_num == preset_num )
                {
                    if( list == synth->preset_amp_offset_list )
                        synth->preset_amp_offset_list = list->next;

                    if( preList != NULL )
                        preList->next = list->next;

                    delete_fluid_preset_amp_offset_list( list );
...
void fluid_synth_delete_preset_amp_offsets_all(fluid_synth_t* synth)
{
    fluid_return_if_fail (synth != NULL);
    ...
    synth->preset_amp_offset_list = NULL;
    delete_fluid_hashtable(synth->preset_amp_offset_hash);
...
static void
fluid_synth_update_channel_gain(fluid_synth_t* synth, int chan)
{
    fluid_voice_t* voice = NULL;
    fluid_real_t chan_gain = 1.0;
    fluid_channel_t* channel = NULL;
    int i;
    ...
    if( chan >= 0 && chan < synth->midi_channels )
    {
        ...
        for (i = 0; i < synth->polyphony; i++)
        {
            ...
            if (fluid_voice_is_playing(voice))
            {
                if( voice->chan_gain != chan_gain )
                    fluid_voice_set_channel_gain( voice, chan_gain );
            }
        }
    }
}

int fluid_synth_set_channel_gain(fluid_synth_t* synth, int channum, double chan_gain)
{
    fluid_channel_t* channel = NULL;
    fluid_voice_t* voice = NULL;
    int i = 0;
    ...
    fluid_return_val_if_fail (synth != NULL, 0);
    fluid_return_val_if_fail (chan_gain >= 0, 0);
    fluid_synth_api_enter(synth);

    if( channum >= 0 && channum < synth->midi_channels )
    {
        synth->channel_gain[channum] = chan_gain;
        channel = synth->channel[channum];
        if( channel != NULL )
        {
            channel->chan_gain = chan_gain;

            for (i = 0; i < synth->polyphony; i++)
            {
                voice = synth->voice[i];
                if( voice == NULL || voice->chan != channum )
                    continue;
...
double fluid_synth_get_channel_gain(fluid_synth_t* synth, int channum)
{
    double chan_gain = 1.0;

    fluid_return_val_if_fail (synth != NULL, 0);

    if( channum >= 0 && channum < synth->midi_channels ) {
        chan_gain = synth->channel_gain[channum];
    }
