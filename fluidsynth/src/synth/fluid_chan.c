...
new_fluid_channel(fluid_synth_t* synth, int num)
{
  ...
  FLUID_MEMSET(chan, 0, sizeof(fluid_channel_t));
  ...
  chan->tuning = NULL;
  chan->mute = FALSE;
  chan->chan_gain = chan->synth->channel_gain[num];
...
fluid_channel_init(fluid_channel_t* chan)
{
  ...
  chan->nrpn_active = 0;
  chan->mute = FALSE;
  chan->chan_gain = chan->synth->channel_gain[chan->channum];
...
{
  if (chan == NULL) {
    FLUID_LOG(FLUID_ERR, "channel is null");
    return;
  }
