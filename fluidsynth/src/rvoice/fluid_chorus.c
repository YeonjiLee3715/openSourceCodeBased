...
#define MIN_SPEED_HZ  0.000001  /* min lfo frequency (Hz) */
...
{
    fluid_real_t w = 2 * M_PI * freq / sample_rate; /* initial angle */
    fluid_real_t a;
    ...
    mod->buffer2 = sin(a - w); /* y(n-1) = sin(-intial angle) */
    mod->buffer1 = sin(a); /* y(n) = sin(initial phase) */
    mod->reset_buffer2 = sin(M_PI / 2 - w); /* reset value for PI/2 */
}
                             ...
                             fluid_real_t *left_out, fluid_real_t *right_out)
{
    ...
    for(sample_index = 0; sample_index < FLUID_BUFSIZE; sample_index++)
    {
        ...
        */
        ...
        if( left_out[sample_index] < -1 )
            left_out[sample_index] = -1;
        else if( left_out[sample_index] > 1 )
            left_out[sample_index] = 1;

        right_out[sample_index] += d_out[1] * chorus->wet1  + d_out[0] * chorus->wet2;
        if( right_out[sample_index] < -1 )
            right_out[sample_index] = -1;
        else if( right_out[sample_index] > 1 )
            right_out[sample_index] = 1;
