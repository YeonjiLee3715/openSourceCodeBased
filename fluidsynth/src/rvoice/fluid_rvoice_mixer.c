...
fluid_mix_one(fluid_rvoice_t* rvoice, fluid_real_t** bufs, unsigned int bufcount, int blockcount)
{
  ...
  while( rvoice->dsp.seek_ticks > 0 )
  {
    fluid_rvoice_write(rvoice, local_buf);
    FLUID_MEMSET(&local_buf, 0, FLUID_BUFSIZE*sizeof(fluid_real_t));
    rvoice->dsp.seek_ticks -= FLUID_BUFSIZE;
  }
			       ...
			       unsigned int bufcount)
{
  ...
  s = fluid_mix_one(voice, bufs, bufcount, buffers->mixer->current_blockcount);
...
{
  int i;
  fluid_real_t* del_buffer = NULL;

  if( buffers->finished_voices != NULL )
  {
  ...
  if (buffers->left_buf != NULL) {
    for (i = 0; i < buffers->buf_count; i++) {
      del_buffer = buffers->left_buf[i];
      buffers->left_buf[i] = NULL;
      if ( del_buffer != NULL) {
	    FLUID_FREE(del_buffer);
        del_buffer = NULL;
      }
    }
    FLUID_FREE(buffers->left_buf);
    buffers->left_buf = NULL;
  }

  if (buffers->right_buf != NULL) {
    for (i = 0; i < buffers->buf_count; i++) {
      del_buffer = buffers->right_buf[i];
      buffers->right_buf[i] = NULL;
      if ( del_buffer != NULL) {
	    FLUID_FREE(del_buffer);
        del_buffer = NULL;
      }
    ...
    buffers->right_buf = NULL;
  }

  if (buffers->fx_left_buf != NULL) {
    for (i = 0; i < buffers->fx_buf_count; i++) {
      del_buffer = buffers->fx_left_buf[i];
      buffers->fx_left_buf[i] = NULL;
      if ( del_buffer != NULL) {
        FLUID_FREE(del_buffer);
        del_buffer = NULL;
      }
    ...
    buffers->fx_left_buf = NULL;
  }

  if (buffers->fx_right_buf != NULL) {
    for (i = 0; i < buffers->fx_buf_count; i++) {
      del_buffer = buffers->fx_right_buf[i];
      buffers->fx_right_buf[i] = NULL;
      if ( del_buffer != NULL) {
        FLUID_FREE(del_buffer);
        del_buffer = NULL;
      }
    ...
    buffers->fx_right_buf = NULL;
  }  
}

int
fluid_rvoice_mixer_event_reset(fluid_rvoice_mixer_t *mixer)
{
  int result = FLUID_FAILED;
  int i;

  fluid_return_val_if_fail(mixer != NULL, result);
...
void delete_fluid_rvoice_mixer(fluid_rvoice_mixer_t* mixer)
{
  ...
  if (mixer->fx.reverb)
  {
    ...
    mixer->fx.reverb = NULL;
  }
  if (mixer->fx.chorus)
  {
    delete_fluid_chorus( mixer->fx.chorus );
    mixer->fx.chorus = NULL;
  }

  if(mixer->rvoices != NULL)
  {
