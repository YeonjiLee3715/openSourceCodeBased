  ...
  else
  {
    ...
    target_amp = fluid_atten2amp (voice->dsp.attenuation)
      * fluid_cb2amp (960.0f * (1.0f - fluid_adsr_env_get_val(&voice->envlfo.volenv))
		      + fluid_lfo_get_val(&voice->envlfo.modlfo) * -voice->envlfo.modlfo_to_vol) /*여기에 곱*/;
...
fluid_rvoice_check_sample_sanity(fluid_rvoice_t* voice)
{
	...
	if (voice->dsp.sample->amplitude_that_reaches_noise_floor_is_valid && voice->dsp.samplemode == FLUID_LOOP_DURING_RELEASE){
	  voice->dsp.amplitude_that_reaches_noise_floor_loop=voice->dsp.sample->amplitude_that_reaches_noise_floor / (voice->dsp.synth_gain*voice->dsp.preset_amplitude_offset);
	} else {
	  /* Worst case */
...
void
fluid_rvoice_seek_to_ticks(fluid_rvoice_t* voice, int ticks)
{
  voice->dsp.seek_ticks = ticks*FLUID_BUFSIZE;
}
...
fluid_rvoice_set_synth_gain(fluid_rvoice_t* voice, fluid_real_t value)
{
   ...
   */
  voice->dsp.amplitude_that_reaches_noise_floor_nonloop = FLUID_NOISE_FLOOR / (value*voice->dsp.preset_amplitude_offset*voice->dsp.chan_gain);
  voice->dsp.amplitude_that_reaches_noise_floor_loop = FLUID_NOISE_FLOOR / (value*voice->dsp.preset_amplitude_offset*voice->dsp.chan_gain);
  voice->dsp.check_sample_sanity_flag |= FLUID_SAMPLESANITY_CHECK;
}

void
fluid_rvoice_set_preset_amplitude_offset(fluid_rvoice_t* voice, fluid_real_t value)
{
  voice->dsp.preset_amplitude_offset = value;

  voice->dsp.amplitude_that_reaches_noise_floor_nonloop = FLUID_NOISE_FLOOR / (voice->dsp.synth_gain*value);
  voice->dsp.amplitude_that_reaches_noise_floor_loop = FLUID_NOISE_FLOOR / (voice->dsp.synth_gain*value);
  voice->dsp.check_sample_sanity_flag |= FLUID_SAMPLESANITY_CHECK;

}

void
fluid_rvoice_set_channel_gain(fluid_rvoice_t* voice, fluid_real_t value)
{
  voice->dsp.chan_gain = value;

  voice->dsp.amplitude_that_reaches_noise_floor_nonloop = FLUID_NOISE_FLOOR / (voice->dsp.synth_gain*voice->dsp.preset_amplitude_offset*value);
  voice->dsp.amplitude_that_reaches_noise_floor_loop = FLUID_NOISE_FLOOR / (voice->dsp.synth_gain*voice->dsp.preset_amplitude_offset*value);
  voice->dsp.check_sample_sanity_flag |= FLUID_SAMPLESANITY_CHECK;
