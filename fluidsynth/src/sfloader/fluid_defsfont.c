...
fluid_sfloader_t* new_fluid_defsfloader(fluid_settings_t* settings)
{
  ...
  FLUID_MEMSET(loader, 0, sizeof(fluid_sfloader_t));

  loader->data = settings;
  fluid_defsfloader_set_callbacks(loader,
                                  default_fopen,
                                  safe_fread,
                                  safe_fseek,
                                  default_ftell,
 ...
 */
int fluid_defsfloader_set_callbacks(fluid_sfloader_t* loader,
...
fluid_sfont_t* fluid_defsfloader_load(fluid_sfloader_t* loader, const char* filename)
{
  ...
  FLUID_MEMSET(sfont, 0, sizeof(fluid_sfont_t));
...
fluid_defsfont_sfont_get_preset(fluid_sfont_t* sfont, unsigned int bank, unsigned int prenum)
{
  ...
  FLUID_MEMSET(preset, 0, sizeof(fluid_preset_t));
  ...
  preset->noteon_with_seek = fluid_defpreset_preset_noteon_with_seek;
  preset->notify = NULL;
...
int fluid_defpreset_preset_noteon_with_seek(fluid_preset_t* preset, fluid_synth_t* synth,
                                  int chan, int key, int vel, int ticks)
{
  return fluid_defpreset_noteon_with_seek((fluid_defpreset_t*) preset->data, synth, chan, key, vel, ticks);
}
...
fluid_defsfont_t* new_fluid_defsfont(fluid_settings_t* settings)
{
  ...
  FLUID_MEMSET(sfont, 0, sizeof(fluid_defsfont_t));
  ...
  for (i = 0; i < sfont->preset_stack_capacity; i++) {
    ...
    FLUID_MEMSET(sfont->preset_stack[i], 0, sizeof(fluid_preset_t));
    sfont->preset_stack_size++;
  }
...
int fluid_defsfont_load(fluid_defsfont_t* sfont, const fluid_file_callbacks_t* fcbs, const char* file)
{
  ...
  FLUID_MEMSET(sfont->filename, 0, sizeof(char)*(1+FLUID_STRLEN(file)));
...
new_fluid_defpreset(fluid_defsfont_t* sfont)
{
  ...
  FLUID_MEMSET(preset, 0, sizeof(fluid_defpreset_t));
...
/*
 * fluid_defpreset_noteon_with_seek
 */
int
fluid_defpreset_noteon_with_seek(fluid_defpreset_t* preset, fluid_synth_t* synth, int chan, int key, int vel, int ticks)
{
  fluid_preset_zone_t *preset_zone, *global_preset_zone;
  fluid_inst_t* inst;
  fluid_inst_zone_t *inst_zone, *global_inst_zone;
    ...
    if (fluid_preset_zone_inside_range(preset_zone, key, vel)) {
      ...
      while (inst_zone != NULL) {
        ...
        if (fluid_inst_zone_inside_range(inst_zone, key, vel) && (sample != NULL)) {
          ...
          /* add the synthesis process to the synthesis loop. */
            fluid_synth_start_voice_with_seek(synth, voice, ticks);
...
new_fluid_preset_zone(char *name)
{
  ...
  FLUID_MEMSET(zone, 0, sizeof (fluid_preset_zone_t));
  ...
  FLUID_MEMSET(zone->name, 0, sizeof(char)*size);
...
new_fluid_inst()
{
  ...
  FLUID_MEMSET(inst, 0, sizeof(fluid_inst_t));
...
new_fluid_inst_zone(char* name)
{
  ...
  FLUID_MEMSET(zone, 0, sizeof(fluid_inst_zone_t));
...
new_fluid_sample()
{
  ...
  FLUID_MEMSET(sample, 0, sizeof (fluid_sample_t));
  sample->valid = 1;
...
fluid_sample_import_sfont(fluid_sample_t* sample, SFSample* sfsample, fluid_defsfont_t* sfont)
{
  ...
  if (sample->sampletype & FLUID_SAMPLETYPE_OGG_VORBIS)
  {
    ...
    FLUID_ASSERT(sample->userdata == NULL);
...
static int load_body (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int read_listchunk (SFChunk * chunk, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int process_info (int size, SFData * sf, FILE* fd, const fluid_file_callbacks_t* fcbs);
static int process_sdta (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int pdtahelper (unsigned int expid, unsigned int reclen, SFChunk * chunk,
                       int * size, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int process_pdta (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_phdr (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_pbag (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_pmod (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_pgen (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_ihdr (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_ibag (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_imod (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_igen (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
static int load_shdr (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs);
...
sfload_file (const char * fname, const fluid_file_callbacks_t* fcbs)
{
  ...
  if (!err)
    {
      FLUID_MEMSET (sf, 0, sizeof (SFData));	/* zero sfdata */
...
static int
load_body (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  SFChunk chunk;
...
static int
read_listchunk (SFChunk * chunk, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
...
static int
process_info (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  SFChunk chunk;
  unsigned char id;
  char *item;
...
static int
process_sdta (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  SFChunk chunk;
...
static int
process_pdta (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  SFChunk chunk;
...
static int
load_phdr (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  int i, i2;
  ...
  for (; i > 0; i--)
    {				/* load all preset headers */
      ...
      FLUID_MEMSET(p, 0, sizeof(SFPreset));
...
/* preset bag loader */
static int
load_pbag (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  fluid_list_t *p, *p2;
  SFZone *z, *pz = NULL;
  ...
  while (p)
	...
	{			/* traverse preset's zones */
	  ...
	  z = FLUID_NEW (SFZone);
      FLUID_MEMSET(z, 0, sizeof(SFZone));
...
static int
load_pmod (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
 ...
 * ------------------------------------------------------------------- */
static int
load_pgen (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
...
static int
load_ihdr (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  int i, i2;
  ...
  for (i = 0; i < size; i++)
    {				/* load all instrument headers */
      p = FLUID_NEW (SFInst);
      FLUID_MEMSET(p, 0, sizeof(SFInst));
...
static int
load_ibag (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  fluid_list_t *p, *p2;
  SFZone *z, *pz = NULL;
...
static int
load_imod (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  fluid_list_t *p, *p2, *p3;
  SFMod *m;
...
static int
load_igen (int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  fluid_list_t *p, *p2, *p3, *dup, **hz = NULL;
  SFZone *z;
  SFGen *g;
...
static int
load_shdr (unsigned int size, SFData * sf, FILE * fd, const fluid_file_callbacks_t* fcbs)
{
  unsigned int i;
  SFSample *p;
  ...
  for (i = 0; i < size; i++)
    {
      ...
      FLUID_MEMSET(p, 0, sizeof(SFSample));
...
{
  if (FLUID_FREAD (buf, count, 1, fd) != 1)
